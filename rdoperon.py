...
def predict_region(reads_df:pd.DataFrame, bin_width:float, num_bins:int):
    ...
    subtranscript_df = pd.DataFrame(columns=TRANSCRIPT_COLS)
    ...
    for row in possible_transcripts_df.itertuples():
        ...
        if not interior_reads_df.empty:
    ...
    while True:
        ...
        for row in indiv_assessment_frame_df.itertuples():
            ...
            predicted_frame_df = predict_linear_domain(row, tstart, tend)
    ...
    final_transcripts_df = possible_transcripts_df[possible_transcripts_df["id"].isin((list(final_transcripts)))].copy()
    if not orientation == "+":
    ...
    for row in final_transcripts_df.itertuples():
        chr = reads_df["chr"].unique().tolist()[0]
...
def assign_reads_to_region(row, reads_df):
    # Assign this read to a region
    # Read must fall within region boundaries and be in the same strand
    mask = (row["start"] <= reads_df["start"]) & (reads_df["end"] <= row["end"]) & (row["strand"] == reads_df["strand"])
...
def create_depth_df(reads_df, total_range_s, bin_labels):
    depth_df = pd.DataFrame(total_range_s, columns=["rel_position"])
    depth_reads_df = pd.merge(depth_df, reads_df, how="cross")
    depth_df["raw_depth"] = get_raw_depth(depth_reads_df)
    depth_df["transcript"] = depth_reads_df.groupby("rel_position").apply(get_transcript).tolist()
...
def get_transcript(reads_df):
    """Get name of trascript for this collection of reads. Assumes only one transcript exists."""
    transcript_list = reads_df["chr"].unique()
    if not len(transcript_list):
        return ""
    if len(transcript_list) > 1:
        raise "Multiple transcript chr identifiers in this reads dataframe"
    return transcript_list[0]

#### Depth Evaluation Function
def predict_linear_domain(row, tstart, tend):
    # row - dstart/dend - depth region
    # tstart/tend - transcript
    # row - a - coefficient
    # row - b - y-intercept

    data = [{"rel_position":i + tstart, "depth":0} for i in range(tend - tstart)]
    ...
    depth_frame_df["in_range"] = depth_frame_df["rel_position"].apply(is_in_range, args=(row.dstart, row.dend))
    in_range_df = depth_frame_df[depth_frame_df["in_range"]]

    # prediction is y = ax + b
    depth_frame_df.loc[depth_frame_df["in_range"], "depth"] = (in_range_df["rel_position"] * row.a) + row.b
